...
int encode_video(StreamingContext *decoder, StreamingContext *encoder, AVFrame *input_frame)
{
    ...
    if (input_frame != NULL)
    {
        // for(int y=0;y<decoder->video_avcc->height;y++) {
        //     for(int x=0;x<decoder->video_avcc->width;x++) {
        //         input_frame->data[0][y * input_frame->linesize[0] + x] += 20;
        //     }
        // }
    

        // logging("herererererererere");
        // logging("height %i", encoder->video_avcc->height);
        /* Cb and Cr */
        for(int y = 0; y < encoder->video_avcc->height / 2; y++) {
            for(int x = 0; x < encoder->video_avcc->width / 2; x++) {
                input_frame->data[1][y * input_frame->linesize[1] + x] = 255 / 2;
                input_frame->data[2][y * input_frame->linesize[2] + x] = 255 / 2;
            }
        }
    }
    // end my
